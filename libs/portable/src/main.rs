...
const APP_PREFIX: &str = "OneSupportManager";
const APPNAME_RUNTIME_ENV_KEY: &str = "ONESUPPORT_APPNAME";
#[cfg(windows)]
...
fn main() {
    ...
    // Force silent install
    if !args.contains(&"--silent-install".to_owned()) {
    ...
    if let Some(exe) = setup(
        ...
        // Check if elevation is needed for silent install
        #[cfg(windows)]
        if args.contains(&"--silent-install".to_owned()) {
             use std::os::windows::process::CommandExt;
             // Naive check: if we can't write to Program Files, we might not be admin.
             // Better: explicitly elevate via runas if not already.
             // But here we are launching the *extracted* exe. 
             // We can use ShellExecute to run it as admin.
             
             let params = args.join(" ");
             let file = std::ffi::CString::new(exe.to_string_lossy().to_string()).unwrap();
             let params = std::ffi::CString::new(params).unwrap();
             let verb = std::ffi::CString::new("runas").unwrap();
             ...
             return; // Exit wrapper, let escalated process handle it
        }
...
mod win {
    ...
    pub const WIN_TOPMOST_INJECTED_PROCESS_EXE: &'static str = "RuntimeBroker_onesupport.exe";
