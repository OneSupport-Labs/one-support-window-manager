...
hbb_name = 'OneSupportManager' + ('.exe' if windows else '')
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    src_exe = f'../../{flutter_build_dir_2}OneSupportManager.exe'
    dst_exe = f'../../{flutter_build_dir_2}OneSupportManager.exe'
    # Rename if source exists (it should), but handle re-run case where dst might already exist
    if os.path.exists(src_exe) and src_exe != dst_exe:
        if os.path.exists(dst_exe):
            os.remove(dst_exe)
        os.rename(src_exe, dst_exe)
    
    system2(f'{sys.executable} ./generate.py -f ../../{flutter_build_dir_2} -o . -e {dst_exe}')
    os.chdir('../..')
    ...
    os.rename('./rustdesk_portable.exe', f'./OneSupportManager-{version}.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/OneSupportManager-{version}.exe')
