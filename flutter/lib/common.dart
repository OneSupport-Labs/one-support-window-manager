...
class MyTheme {
  ...
  static const Color accent = Color(0xFF003BED);
  static const Color accent50 = Color(0xFF809DF6);
  static const Color accent80 = Color(0xFFCCD8FB);
  ...
  static const Color button = Color(0xFF003BED);
...
void checkUpdate() async {
  if (isWindows) {
    try {
      ...
      // 2. Call API
      final apiUrl = 'https://1support.co.kr/api/version/check';
      final response = await http_client.post(Uri.parse(apiUrl),
          headers: {'Content-Type': 'application/json'},
          body: jsonEncode({'platform': 'windows', 'type': 'manager', 'version': version}));
      ...
      if (response.statusCode == 200) {
        ...
        if (needsUpdate && fileUrl != null) {
          showUpdateDialog(forceUpdate, fileUrl, data['latestVersion']);
        }
      }
    } catch (e) {
...
void showUpdateDialog(bool forceUpdate, String url, String? latestVersion) {
  // Use a slight delay to ensure context is available if called immediately on startup
        ...
        return WillPopScope(
          ...
          child: AlertDialog(
            ...
            content: Text(forceUpdate
                ? '필수 업데이트가 있습니다.\n최신 버전으로 업데이트해주세요. ($latestVersion)'
                : '새로운 버전이 있습니다.\n업데이트 하시겠습니까? ($latestVersion)'),
            actions: [
              if (!forceUpdate)
                TextButton(
