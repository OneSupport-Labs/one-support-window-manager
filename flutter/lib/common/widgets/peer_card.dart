...
class _PeerCardState extends State<_PeerCard>
      ...
      children: [
        Container(
            decoration: BoxDecoration(
              color: str2color('${peer.id}${peer.platform}', 0x7f).withOpacity(0.2), // Softer background
              borderRadius: BorderRadius.circular(50), // Circular
            ),
            alignment: Alignment.center,
            width: isPortrait ? 50 : 36, // Reduced width for list view
            height: isPortrait ? 50 : 36, // Reduced height to fit 45px row
            margin: EdgeInsets.only(right: 8), // Right margin only, no vertical margin expansion
            child: Stack(
              children: [
                getPlatformImage(peer.platform, size: isPortrait ? 30 : 20) // Smaller icon
                    .paddingAll(6),
                if (_shouldBuildPasswordIcon(peer))
                  Positioned(
                    top: 1,
                    left: 1,
                    child: Icon(Icons.key, size: 6, color: Colors.black54),
                  ),
              ],
            )),
        Expanded(
  ...
  Widget _buildPeerCard(
    ...
    final child = Card(
      ...
      margin: EdgeInsets.all(4), // Little margin for shadow
      child: Obx(
        () => Container(
          foregroundDecoration: deco.value,
          decoration: BoxDecoration(
          ...
          child: ClipRRect(
            ...
            child: Column(
              ...
              children: [
                Expanded(
                  child: Container(
                    // Removed solid colored background
                    ...
                    child: Center( // Center the content
                      child: Column(
                            ...
                            children: [
                              Container(
                                width: 50, // Reduced from 70 to prevent overflow
                                height: 50,
                                decoration: BoxDecoration(
                                  color: str2color('${peer.id}${peer.platform}', 0x7f).withOpacity(0.2), // Soft circle bg
                                  shape: BoxShape.circle,
                                ),
                                padding: const EdgeInsets.all(10), // Reduced padding
                                child: getPlatformImage(peer.platform, size: 30), // Reduced icon size
                              ),
                              SizedBox(height: 4), // Reduced spacer
                              Row(
                                children: [
                                  Expanded(
                                    child: Tooltip(
                ...
                Container(
                  color: Theme.of(context).brightness == Brightness.light ? Colors.grey[50] : Colors.black26, // Subtle footer bg
                  child: Row(
