...
impl Client {
    ...
    async fn _start_inner(
        ...
        msg_out.set_punch_hole_request(PunchHoleRequest {
            ...
            custom_access_token: crate::ui_interface::get_local_option("access_token_c".to_string()),
            username: crate::ui_interface::get_local_option("user_name_c".to_string()),
                    ...
                    Some(rendezvous_message::Union::PunchHoleResponse(ph)) => {
                        if ph.socket_addr.is_empty() {
                            ...
                            match ph.failure.enum_value() {
                                ...
                                Ok(punch_hole_response::Failure::TOKEN_EXPIRED) => {
                                    log::info!("Token expired, refreshing...");
                                    Self::refresh_access_token().await?;
                                    msg_out.mut_punch_hole_request().custom_access_token = crate::ui_interface::get_local_option("access_token_c".to_string());
                                    log::info!("Token refreshed, reconnecting socket and retrying punch hole...");
                                    if let Ok(s) = connect_tcp(&*rendezvous_server, CONNECT_TIMEOUT).await {
                                        socket = s;
                                        if !key.is_empty() && !token.is_empty() {
                                            if let Err(e) = secure_tcp(&mut socket, &key).await {
                                                 log::warn!("Failed to secure tcp on retry: {}", e);
                                            }
                                        }
                                    } else {
                                        log::warn!("Failed to reconnect socket on retry");
                                    }
                                    continue;
                                }
                                Ok(punch_hole_response::Failure::TOKEN_INVALID) => {
                                    log::error!("Token invalid, forcing logout");
                                    #[cfg(feature = "flutter")]
                                    crate::flutter::push_global_event("token_invalid", "".to_owned());
                                    bail!("Token invalid");
                                }
...
async fn refresh_access_token() -> ResultType<()> {
    let refresh_token = crate::ui_interface::get_local_option("refresh_token_c".to_owned());
    let device_id = crate::ui_interface::get_local_option("device_id_c".to_owned());
    if refresh_token.is_empty() {
        bail!("No refresh token");
    }
    log::info!("Refreshing access token...");
    let client = reqwest::Client::new();
    let res = client.post("https://1support.co.kr/api/auth/refresh")
    ...
    if let (Some(access_token), Some(refresh_token)) = (body["accessToken"].as_str(), body["refreshToken"].as_str()) {
        crate::ui_interface::set_local_option("access_token_c".to_owned(), access_token.to_owned());
        crate::ui_interface::set_local_option("refresh_token_c".to_owned(), refresh_token.to_owned());
