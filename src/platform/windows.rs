...
pub fn start_os_service() {
    ...
    // Check and register the URL scheme
    if let Ok(exe_path) = std::env::current_exe() {
        let exe_path_str = exe_path.to_string_lossy();
        let hkcr = RegKey::predef(HKEY_CLASSES_ROOT);
        let scheme = "onesupportmanager";
        
        let path = format!("{}", scheme);
        if let Ok((key, disp)) = hkcr.create_subkey(&path) {
...
pub fn install_me(options: &str, path: String, silent: bool, debug: bool) -> ResultType<()> {
    // Prevent redundant installation
    // if get_reg("Version") == crate::VERSION {
...
reg add HKEY_CLASSES_ROOT\\onesupportmanager /f
reg add HKEY_CLASSES_ROOT\\onesupportmanager /ve /d \"URL:onesupportmanager Protocol\" /f
reg add HKEY_CLASSES_ROOT\\onesupportmanager /v \"URL Protocol\" /d \"\" /f
reg add HKEY_CLASSES_ROOT\\onesupportmanager\\shell\\open\\command /ve /d \"\\\"{exe}\\\" \\\"%1\\\"\" /f
cscript \"{mk_shortcut}\"
...
{sleep}
    ...
    let splash_hwnd = start_install_splash();
    // Reverted to silent (false) as requested, but now showing splash screen
    if let Err(err) = run_cmds(cmds, debug, "install") {
        stop_install_splash(splash_hwnd);
        return Err(err);
    }
    stop_install_splash(splash_hwnd);
    // run_after_run_cmds(silent);
    // Explicitly start the tray after installation to ensure it runs even if config overrides existed
    if !silent {
...
{sleep}
    ...
    let splash_hwnd = start_install_splash();
    // Reverted to silent (false) as requested, but now showing splash screen
    if let Err(err) = run_cmds(cmds, debug, "update") {
        stop_install_splash(splash_hwnd);
        return Err(err);
    }
    stop_install_splash(splash_hwnd);
...
unsafe extern "system" fn splash_wnd_proc(
    ...
    match msg {
        WM_PAINT => {
            ...
            // Background
            let brush = CreateSolidBrush(0xFFFFFF); // White
            FillRect(hdc, rect, brush);
            DeleteObject(brush as _);

            // Text
            let text = "프로그램 실행중...".to_string();
            let text_wide: Vec<u16> = text.encode_utf16().chain(std::iter::once(0)).collect();
            
            // Simple centering logic or just DrawText
            let mut rect_text = *rect;
            SetBkMode(hdc, TRANSPARENT as _);
            SetTextColor(hdc, 0x000000); // Black
            DrawTextW(
                hdc,
                text_wide.as_ptr(),
                -1,
...
fn start_install_splash() -> usize {
    let (tx, rx) = std::sync::mpsc::channel();
    std::thread::spawn(move || {
        unsafe {
            let h_instance = GetModuleHandleW(std::ptr::null_mut());
            let class_name = "OneSupportSplash\0".encode_utf16().collect::<Vec<u16>>();
            ...
            let hwnd = CreateWindowExW(
                ...
                x, y, width, height,
                std::ptr::null_mut(), std::ptr::null_mut(), h_instance, std::ptr::null_mut()
            );
...
fn stop_install_splash(hwnd: usize) {
    if hwnd != 0 {
